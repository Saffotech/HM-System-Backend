...
from Services.nurse_emergency_alert_notify import (
    notify_assigned_nurse_alert,
    notify_doctor_critical_auto_alert,
)
...
def _generate_alert_uid(db: Session):
    ...
    next_number = 1
    if last_alert:
        next_number = last_alert.id + 1
    return f"EA-{current_year}-{str(next_number).zfill(6)}"


def _get_patient_bed_snapshot(db: Session, patient_id: int):
    return nh.patient_bed_snapshot(db, patient_id)
...
from Services.nurse_emergency_alert_actions import (  # noqa: E402
