"""OPD Billing profile service — GET/PUT profile and image upload/delete."""
import logging
import os
import shutil
import uuid
...
def _get_upload_dir() -> Path:
    upload_dir = Path(
        os.getenv("OPD_BILLING_PROFILE_UPLOAD_DIR", "uploads/opd_billing_image")
    )
...
def _assert_opd_billing(user: User) -> None:
    role_name = user.role_obj.name if user.role_obj else None
    if role_name != OPD_BILLING_ROLE:
        raise HTTPException(
            status_code=403,
            detail="Only OPD billing staff can access this endpoint",
        )
...
def compute_profile_completed(user: User, profile: OpdBillingProfile) -> bool:
...
def compute_profile_completion_percentage(
    user: User, profile: OpdBillingProfile
) -> int:
...
def _get_opd_billing_user(db: Session, user_id: int) -> User:
    user = (
        db.query(User)
        .options(
    ...
    _assert_opd_billing(user)
    return user


def _get_profile_or_404(user: User) -> OpdBillingProfile:
    profile = user.opd_billing_profile
    if not profile:
        raise HTTPException(
            status_code=404,
            detail="OPD billing profile not found. Contact admin.",
        )
    return profile


def _to_response(user: User, profile: OpdBillingProfile) -> OpdBillingProfileResponse:
    languages = profile.languages if isinstance(profile.languages, list) else []
    ...
    return OpdBillingProfileResponse(
        user_id=user.id,
...
def get_opd_billing_profile(
    ...
    user = _get_opd_billing_user(db, current_user.id)
...
def update_opd_billing_profile(
    ...
    logger.info("OPD billing user %s updated profile", current_user.id)
    return _to_response(user, profile)
...
def upload_profile_image(
    ...
    try:
        ...
        logger.exception(
            "Failed to save profile image for OPD billing user %s", current_user.id
        )
    ...
    logger.info("OPD billing user %s uploaded profile image", current_user.id)
    return {
...
def delete_profile_image(db: Session, current_user: User) -> dict:
    ...
    logger.info("OPD billing user %s deleted profile image", current_user.id)
    return {
...
def create_empty_opd_billing_profile(
    db: Session,
    user_id: int,
) -> OpdBillingProfile:
    profile = OpdBillingProfile(
        user_id=user_id,
        languages=[],
