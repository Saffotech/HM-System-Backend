"""Pharmacist profile service — GET/PUT profile and image upload/delete."""
import logging
import os
import shutil
import uuid
...
def _get_upload_dir() -> Path:
    upload_dir = Path(
        os.getenv("PHARMACIST_PROFILE_UPLOAD_DIR", "uploads/pharmacist_image")
    )
...
def _assert_pharmacist(user: User) -> None:
    role_name = user.role_obj.name if user.role_obj else None
    if role_name != PHARMACIST_ROLE:
        raise HTTPException(
            status_code=403,
            detail="Only pharmacists can access this endpoint",
        )
...
def compute_profile_completed(profile: PharmacistProfile) -> bool:
    """Minimum completion: pharmacy license + qualification."""
    return bool(profile.qualification and profile.license_number)


def compute_profile_completion_percentage(
    user: User, profile: PharmacistProfile
) -> int:
...
def _get_pharmacist_user(db: Session, user_id: int) -> User:
    user = (
        db.query(User)
        .options(
    ...
    _assert_pharmacist(user)
    return user


def _get_profile_or_404(user: User) -> PharmacistProfile:
    profile = user.pharmacist_profile
    if not profile:
        raise HTTPException(
            status_code=404,
            detail="Pharmacist profile not found. Contact admin.",
        )
    return profile


def _to_response(user: User, profile: PharmacistProfile) -> PharmacistProfileResponse:
    languages = profile.languages if isinstance(profile.languages, list) else []
...
def get_pharmacist_profile(
    ...
    user = _get_pharmacist_user(db, current_user.id)
...
def update_pharmacist_profile(
    ...
    profile_fields = ("qualification", "license_number", "experience_years", "bio")
    user_fields = (
        "phone",
        "phone_code",
        "date_of_birth",
    ...
    logger.info("Pharmacist %s updated profile", current_user.id)
    return _to_response(user, profile)
...
def upload_profile_image(
    ...
    try:
        ...
        logger.exception(
            "Failed to save profile image for pharmacist %s", current_user.id
        )
    ...
    logger.info("Pharmacist %s uploaded profile image", current_user.id)
    return {
...
def delete_profile_image(db: Session, current_user: User) -> dict:
    ...
    logger.info("Pharmacist %s deleted profile image", current_user.id)
    return {
...
def create_empty_pharmacist_profile(
    db: Session,
    user_id: int,
) -> PharmacistProfile:
    """Create empty pharmacist_profiles row. Caller commits."""
