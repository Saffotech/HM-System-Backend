...
def _get_upload_dir() -> Path:
    upload_dir = Path(
        os.getenv("LAB_TECHNICIAN_PROFILE_UPLOAD_DIR", "uploads/lab_technician_image")
    )
...
def compute_profile_completion_percentage(
    user: User, profile: LabTechnicianProfile
) -> int:
...
def _to_response(user: User, profile: LabTechnicianProfile) -> LabTechnicianProfileResponse:
    languages = profile.languages if isinstance(profile.languages, list) else []
...
def upload_profile_image(
    ...
    try:
        ...
        logger.exception(
            "Failed to save profile image for lab technician %s", current_user.id
        )
...
def create_empty_lab_technician_profile(
    ...
    """Create empty lab_technician_profiles row. Caller commits."""
